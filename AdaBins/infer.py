...
class InferenceHelper:
    def __init__(self, dataset='nyu', device='cuda:0'):
        ...
        if dataset == 'nyu':
            ...
            self.input_height = 320   # put an input_height/input_width pair compatible with
            self.input_width = 420    # the model, used to scale the image if needed
            model = UnetAdaptiveBins.build(n_bins=256, min_val=self.min_depth, max_val=self.max_depth)
            ...
            self.input_width = 420
            model = UnetAdaptiveBins.build(n_bins=256, min_val=self.min_depth, max_val=self.max_depth)
            pretrained_path = "./pretrained/AdaBins_pascal.pt"
        else:
    ...
    def predict_dir(self, test_dir, out_dir):
        ...
        for f in tqdm(all_files):
            ...
            height = image.height
            width = image.width
            # Increase image size when it is too small to work correctly in the model
            if image.height < self.input_height or image.width < self.input_width:
                scaling = max(self.input_height/image.height, self.input_width/image.width)
                image = image.resize((int(math.ceil(image.width*scaling)), int(math.ceil(image.height*scaling))))
            #decrease image size if it is bigger
            elif image.height > self.input_height or image.width > self.input_width:
                scaling = max(self.input_height/image.height, self.input_width/image.width)
                image = image.resize((int(math.ceil(image.width*scaling)), int(math.ceil(image.height*scaling))))
            
            image = np.asarray(image) / 255.
            image = transform(image).unsqueeze(0).float().to(self.device)

            centers, final = self.predict(image)
            final = utils.colorize(torch.from_numpy(final).unsqueeze(0), vmin=None, vmax=None, cmap='inferno')
            depth = Image.fromarray(final.squeeze())
...
if __name__ == '__main__':
    ...
    start = time()
    inferHelper = InferenceHelper(dataset='pascal')
    inferHelper.predict_dir(in_dir, out_dir)
